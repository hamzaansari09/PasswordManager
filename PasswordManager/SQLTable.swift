...
class SQLTable:NSObject {
	...
	func ignoredKeys() -> [String] {
		return ["profileImage","email", "createdAt"]
	}
    
    func onlyGetKeys() -> [String]{
        return ["createdAt"]
    }
	...
	class func rowsFor(sql:String="") -> [SQLTable] {
		var res = [SQLTable]()
		let tmp = self.init()
		let data = tmp.values(isSave: false)
	...
	class func row(filter:String="", order:String="") -> SQLTable? {
		let row = self.init()
		let data = row.values()
		let db = SQLiteDB.sharedInstance
		var sql = "SELECT * FROM \(table)"
